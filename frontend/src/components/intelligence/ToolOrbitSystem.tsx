...
export const ToolOrbitSystem: React.FC<ToolOrbitSystemProps> = ({
  ...
  const activeToolKey = hoveredToolId || focusedToolId;
  ...
  useEffect(() => {
    if (isPaused || activeToolKey) return;

    const interval = setInterval(() => {
      setRotationAngle((previous) => (previous + 0.18) % 360);
    }, 40);

    return () => clearInterval(interval);
  }, [isPaused, activeToolKey]);
  ...
  const getCommand = (tool: any): string => {
    ...
    if (typeof tool?.commands === 'string' && tool.commands.trim()) {
      return tool.commands.trim();
    }

    if (typeof tool?.commandExample === 'string' && tool.commandExample.trim()) {
      return tool.commandExample.trim();
    }

    if (typeof tool?.usage === 'string' && tool.usage.trim()) {
  ...
  const handlePractice = (tool: any) => {
    ...
    const toolIdParam = encodeURIComponent(tool.id || tool._id || '');
  ...
  if (filteredTools.length === 0) {
    return (
      <div className="relative w-full min-h-[500px] flex items-center justify-center">
        <GlassCard glow="emerald" className="p-10 text-center max-w-md">
          <Terminal size={40} className="mx-auto text-[#00ff99] mb-4" />
          <h3 className="font-mono font-bold text-white">
            NO TOOLS IN THIS ORBIT
          </h3>
          <p className="text-xs font-mono text-slate-500 mt-2">
            Select another OSINT category to view available investigation tools.
          </p>
        </GlassCard>
      </div>
    );
    ...
    >
      ...
      >
        <div
          ...
          "
        >
          <Terminal size={25} className="text-[#00ff99] mb-2" />
      ...
      >
        {visibleTools.map((tool: any, index: number) => {
          ...
          const baseScale = 0.84 + depth * 0.16;
          const baseZIndex = 100 + Math.round(depth * 100);

          const toolKey = String(tool.id || tool._id || tool.name || index);
          const isCardActive = activeToolKey === toolKey;
          ...
          return (
            <div
              ...
              onMouseEnter={() => setHoveredToolId(toolKey)}
              onMouseLeave={() => {
                setHoveredToolId((prev) => (prev === toolKey ? null : prev));
              }}
              onFocus={() => setFocusedToolId(toolKey)}
              onBlur={(e) => {
                if (!e.currentTarget.contains(e.relatedTarget as Node)) {
                  setFocusedToolId((prev) => (prev === toolKey ? null : prev));
                }
              }}
              tabIndex={0}
              aria-label={`OSINT Tool: ${tool.name}`}
            >
              {/* LANDSCAPE TOOL CARD */}
              <GlassCard
                glow="emerald"
                className={`
                  ...
                  ${
                    isCardActive
                      ? 'border-[#00ff99] shadow-[0_0_35px_rgba(0,255,153,0.4)] bg-black/90'
                      : 'border-[#00ff99]/20 hover:border-[#00ff99] hover:shadow-[0_0_30px_rgba(0,255,153,0.25)]'
                  }
                `}
              >
                {/* HEADER */}
